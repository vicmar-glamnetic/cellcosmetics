...
import replaceStr from 'gulp-replace';
...
// Build escapeCurlies, liquid support to Emails
gulp.task('escapeCurlies', escapeCurlies);

// Escapes curly braces to properly use liquid template data objects
function escapeCurlies() {
  return gulp.src(['src/pages/**/*.html', '!src/pages/archive/**/*.html'])
    .pipe(replaceStr(/{{.*}}/g, function(match, p1, offset, string) {
...
function pages() {
  return gulp.src(['src/pages/**/*.html', '!src/pages/archive/**/*.html'])
    .pipe(replaceStr(/{{.*}}/g, function(match, p1, offset, string) {
      if(match.indexOf('{{>') >= 0){
        return match;//don't add raw tags if partial
      }else{
        return '{{{{raw}}}}' + match + '{{{{/raw}}}}';
      }
    }))
    ...
    .pipe(replaceStr(/{{{{raw}}}}{{.*}}{{{{\/raw}}}}/g, function(match, p1, offset, string) {
      return match;
    }))
    .pipe(inky())
    .pipe(gulp.dest('dist'));
...
function inliner(css) {
  ...
  var pipe = lazypipe()
    // 1R don't inline css with npm run build, only put it in the head, shopify will inline for us
    // .pipe($.inlineCss, {
    ...
    .pipe($.htmlmin, {
      // 1R ignore liquid logic using '<' and '>' characters which breaks html minifier
